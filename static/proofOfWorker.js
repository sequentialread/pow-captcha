...
onmessage = function(e) {
  const challengeBase64 = e.data;
  if(!challengeBase64) {
    postMessage({type: "error", message: `challenge was not provided`});
  }
  ...
  try {
    ...
    postMessage({type: "error", message: `couldn't decode challenge '${challengeBase64}' as base64: ${err}`});
  }
  try {
    challenge = JSON.parse(challengeJSON);
  } catch (err) {
    postMessage({type: "error", message: `couldn't parse challenge '${challengeJSON}' as json: ${err}`});
  }
  ...
  var i = 0;
  var j = 0;
  var probabilityOfFailurePerAttempt = 1-(1/Math.pow(16, challenge.difficulty.length));
  ...
  while(true) {
    ...
    if(hashHex.endsWith(challenge.difficulty)) {
      postMessage({type: "success", nonce: sjcl.codec.base64.fromBits(challenge.preimage)});
      break
    }
    ...
    if(j == 10) {
      ...
      const probabilityOfSuccessSoFar = 1-Math.pow(probabilityOfFailurePerAttempt, i);

      postMessage({type: "progress", value: probabilityOfSuccessSoFar });
      j = 0
    }
  }
